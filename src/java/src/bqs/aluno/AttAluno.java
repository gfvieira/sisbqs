...
public class AttAluno extends HttpServlet {
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            while (iteratorRequest.hasNext()) {
                ...
                if (key.equals("matricula")) {
                    aluno.setMatricula(request.getParameterValues(key)[0]);
                }
                if (key.equals("nome")) {
            ...
            boolean check = false;
            Aluno alunoTemp = daoaluno.buscaAlunoMATRICULA(aluno.getMatricula());
            if (alunoTemp != null) {
                check = daoaluno.attAluno(aluno);
                if (check == false) {
                    request.setAttribute("mensagem", "NO");
                } else {
                    check = daoaluno.inserAuditorialuno(aluno, "CADASTRO ATUALIZADO COM SUCESSO");
                    if (check == false) {
                        request.setAttribute("mensagem", "NO");
                    } else {
            ...
            request.setAttribute("aluno", aluno.getCpf());
            RequestDispatcher rd = request.getRequestDispatcher("/bqs/aluno/ExibeAluno.jsp");
